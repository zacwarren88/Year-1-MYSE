# Code to analyze ID files and correct based upon match ratio criteria and pulse counts
# folder.IDfiles = folder to search for ID files
# multifolder = recursive


f.aggregateID_NZ <- function(folder.IDfiles, 
  ...
  ID <- bind_rows(lapply(files.IDSUMMARY, read_csv))
  ...
  #read file and bind
  t <- bind_rows(lapply(files.MLE, read_csv, skip = 1))
